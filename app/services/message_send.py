    ...
    try:
        if apply_scope:
            ...
            override_result = await set_radio_flood_scope(
                be, desired_scope, fw_ver=radio_manager.firmware_ver_code
            )
        ...
        if apply_phm:
            ...
            phm_result = await be.set_path_hash_mode(override_phm)
            ...
            for attempt in range(3):
                try:
                    restore_result = await set_radio_flood_scope(
                        be, baseline_scope, fw_ver=radio_manager.firmware_ver_code
                    )
        ...
        if apply_phm:
            ...
            for attempt in range(3):
                try:
                    restore_phm = await be.set_path_hash_mode(baseline_phm)
...
async def _retry_direct_message_until_acked(
    *,
    ...
    while attempt < DM_SEND_MAX_ATTEMPTS:
        ...
        try:
            async with radio_manager.radio_operation("retry_direct_message") as be:
                be = _coerce_be(be)
                ...
                if attempt == DM_SEND_MAX_ATTEMPTS - 1:
                    reset_result = await be.reset_path(contact.public_key)
                    ...
                    refreshed_contact = be.get_contact_by_key_prefix(contact.public_key[:12])
...
async def _channel_echo_watchdog(
    ...
    try:
        ...
        async with radio_manager.radio_operation("echo_watchdog_resend", blocking=False) as be:
            be = _coerce_be(be)
