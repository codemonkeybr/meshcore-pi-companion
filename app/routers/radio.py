...
def _meshcore_from_backend(be) -> Any:
    """Return the underlying MeshCore (raw mc or SpiBackend for SPI).

    Uses isinstance to avoid auto-creating _mc on MagicMock test objects.
    """
...
async def get_radio_config() -> RadioConfigResponse:
    ...
    be = radio_manager.require_connected()
    ...
    async with radio_manager.radio_operation("import_private_key") as be:
        ...
        try:
            await import_private_key_and_refresh_keystore(
                mc,
                key_bytes,
                export_and_store_private_key_fn=lambda _: export_and_store_private_key(be),
            )
...
async def send_advertisement(request: RadioAdvertiseRequest | None = None) -> dict:
    ...
    async with radio_manager.radio_operation("manual_advertisement") as be:
        success = await do_send_advertisement(be, force=True, mode=mode)
...
async def trace_path(request: RadioTraceRequest) -> RadioTraceResponse:
    ...
    async with radio_manager.radio_operation("radio_trace", pause_polling=True) as be:
        mc = _meshcore_from_backend(be)
        local_public_key = str((be.self_info or {}).get("public_key") or "").lower()
        if len(local_public_key) != 64:
            raise HTTPException(status_code=423, detail="Local radio public key is unavailable")
        local_name = (be.self_info or {}).get("name")
...
async def reboot_radio() -> dict:
    ...
    if radio_manager.is_connected:
        ...
        async with radio_manager.radio_operation("reboot_radio") as be:
            mc = _meshcore_from_backend(be)
