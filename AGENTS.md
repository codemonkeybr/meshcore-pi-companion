...
4. **Unauthenticated TCP companion ports**: Virtual companion TCP servers (`app/virtual/companion.py`) accept connections from any client that can reach the configured port. There is no credential exchange or session token. This is intentional — the companion is a trusted-network tool. Operators control exposure via the `bind_address` field in `config.yaml` (e.g., `bind_address: "127.0.0.1"` for local-only access). One connection is allowed per port at a time; additional connections are rejected immediately.
